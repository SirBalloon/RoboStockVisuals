...
// 4-slot channel configuration (34mm slots, 3mm walls)
export const CHANNEL_4_SLOT_CONFIG = {
  length: 120.5,        // 120.5 cm
  height: 7,            // 70mm = 7cm
  wallThickness: 0.3,   // 3mm walls
  slotWidth: 3.4,       // 34mm per slot
  numSlots: 4,
...
function Channel4Slot({ position = [0, 0, 0], onSlotClick, selectedSlot, heightCm }) {
  ...
  // Pattern: wall | slot | wall | slot | wall | slot | wall | slot | wall
  const wallPositions = []
  ...
  return (
    <group position={position}>
      ...
      {/* Divider walls running full length (5 walls creating 4 slots) */}
      ...
      {/* Slot labels (1-4 from right to left) - clickable, under the floor at right end */}
